...
MAKE = make
FULLEXT = Time/Duration/Concise/Localize
BASEEXT = Localize
PARENT_NAME = Time::Duration::Concise
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Time/Duration/Concise.pm \
...
INST_LIBDIR      = $(INST_LIB)/Time/Duration/Concise
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Time/Duration/Concise
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''localize concise time duration string representation.'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   "abstract" : "localize concise time duration string representation.",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Time-Duration-Concise-Localize",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "Time::Seconds" : "1.27"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      },' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "Tie::Hash::LRU" : "0",' >> META_new.json
	$(NOECHO) $(ECHO) '            "Time::Seconds" : "1.27",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "web" : "http://rt.cpan.org/NoAuth/Bugs.html?Dist=Time-Duration-Concise-Localize"' >> META_new.json
	$(NOECHO) $(ECHO) '      },' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>localize concise time duration string representation.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Binary.com &lt;perl@binary.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
